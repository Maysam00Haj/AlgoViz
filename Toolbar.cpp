...
Toolbar::Toolbar() {
    std::vector<std::string> icons = {"add_node", "add_edge", "erase", "change_start_node", "run_bfs", "run_dfs", "run_dijkstra", "end", "reset", "clear_window"};
    std::vector<ButtonId> id_list = {ADD_NODE, ADD_EDGE, ERASE, CHANGE_START_NODE, RUN_BFS, RUN_DFS, RUN_DIJKSTRA, END, RESET, CLEAR_WINDOW};
    for (unsigned int i = 0; i < icons.size(); i++) {
        this->buttons.push_back(std::make_shared<Button>(20, 20 + (55 * i), 150, 40, "./icons/" + icons[i] + ".png", id_list[i]));
    }
...
void Toolbar::render(sf::RenderTarget& target) {
    std::vector<ButtonId> not_to_render_while_running = {ADD_NODE, ADD_EDGE, ERASE, CHANGE_START_NODE, RUN_BFS, RUN_DFS, RUN_DIJKSTRA};
